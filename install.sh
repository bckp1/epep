...
read -rp "Input the license : " -e lcns
    if [ -z $lcns ]; then
        ...
        echo "$lcns" > /usr/local/etc/sansdata/license
fi
# Validating License
...
# Client Data
Name=$(curl -sS ${link}/sansdata/license | grep $license1 | awk '{print $1}')
echo "$Name" > /usr/local/etc/sansdata/client
client=$(cat /usr/local/etc/sansdata/client)
clear
line_atas
